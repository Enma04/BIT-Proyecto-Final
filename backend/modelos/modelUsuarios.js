...
usuariosModel.Modificar = function (data, callback) {
  miModelo.find({ cedula: data.cedula }, {}, (error, documentos) => {
...
usuariosModel.ListarUsuario = function (data, callback) {
  //find({criterio de búsqueda},{datos que se quieren ver o ocultar},{})
  miModelo.find({ _id: data._id }, { __v: 0 }, (error, documentos) => {
    if (error) {
      console.log(error);
      return callback({ state: false, mensaje: error });
    } else {
...
usuariosModel.Login = function (data, callback) {
  //find({criterio de búsqueda}, {datos que se quieren ver o ocultar}, () => {})
  miModelo.find(
    { cedula: data.cedula, password: data.password },
    {
      _id: 0,
      __v: 0,
    },
    (error, documentos) => {
      if (error) {
        ...
        if (documentos.length === 1) {
          return callback({
            state: true,
            mensaje: "Bienvenido: " + documentos[0].nombre,
          });
        } //Fin del if de length
        else {
          return callback({
